...
public interface UserMessageMapper {
    ...
    List<UserMessage> listUserMessages(UserMessageSearchCondition condition);

    // 根据主键id查询消息传输对象
    @Select("select * from user where  user_id = #{userId} and is_read = #{isRead}")
    ...
    //查询当前用户id下，状态为read的消息数量 状态为read的消息数量
    @Select("select * from user_message where  user_id = #{userId} and is_read = #{isRead}")
    List<UserMessage> countUserMessagesByUserIdAndRead(Long userId, int isRead);
