...
public class HiddenDangerServiceImpl implements HiddenDangerService {
    ...
    public JsonResponse synchronization() {
        ...
        for(DayCheck dayCheck : dayChecks) {
            ...
            if(state == true){
                ...
                hiddenDanger.setRectificationFinishTime(dayCheck.getReformTime());
    ...
    public JsonResponse showAll(long roleId, User user) {//通过消息提醒他有待审核的项目，在showAll方法里编辑记录，即日查的5个方法
        ...
        if (roleId == 3) {
            ...
            List<DayCheck> dayChecks = hiddenDangerMapper.showAllByLab(labName,stage);
            return JsonResponse.toSuccess(dayChecks);
        } else if (roleId == 4) {
            String department = user.getDepartment();
            List<DayCheck> dayChecks = hiddenDangerMapper.showAllByDepartment(department,stage);
            return JsonResponse.toSuccess(dayChecks);
        }
        else if( roleId == 5){
            String department = user.getDepartment();
            ...
            List<DayCheck> dayChecks = hiddenDangerMapper.showAll(stage);
            return JsonResponse.toSuccess(dayChecks);
        }
        return JsonResponse.toFailed("查看自查填报记录失败！");
    }
